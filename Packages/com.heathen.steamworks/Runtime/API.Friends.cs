...
namespace Heathen.SteamworksIntegration.API
{
    ...
    {
        /// <summary>
        /// Provides a collection of methods and events for interacting with Steam friends' data and functionality.
        /// </summary>
        /// <remarks>
        /// Enables access to a variety of features such as retrieving friend lists, receiving friend chat messages,
        /// managing rich presence data, tracking co-played games, and more. This class supports operations on the Steam Friends interface.
        /// </remarks>
        public static class Client
        {
            ...
            static void Init()
            {
                _isListeningForFriendMessages = false;
                _pendingLinks = new();
                ...
                _loadedImages = new();
                _userAvatarMapping = new();

                _friendsEnumerateFollowingListT = null;
                _friendsGetFollowerCountT = null;
                _friendsIsFollowingT = null;
#if STEAM_LEGACY || STEAM_161
                _mSetPersonaNameResponseT = null;
#endif
                _avatarImageLoadedT = null;
            }
            ...
            {
                get => _isListeningForFriendMessages;
                set
                {
                    SteamFriends.SetListenForFriendsMessages(value);
                    _isListeningForFriendMessages = value;
                }
            }
            /// <summary>
            ...
            private static bool _isListeningForFriendMessages = false;

            private static List<ImageRequestCallbackLink> _pendingLinks = new List<ImageRequestCallbackLink>();
            private static Dictionary<int, Texture2D> _loadedImages = new Dictionary<int, Texture2D>();
            private static Dictionary<UserData, Texture2D> _userAvatarMapping = new Dictionary<UserData, Texture2D>();
...
#if STEAM_LEGACY || STEAM_161
            private static CallResult<SetPersonaNameResponse_t> _mSetPersonaNameResponseT;
#endif
            
            private static Callback<AvatarImageLoaded_t> _avatarImageLoadedT;

            private static bool _loadingFollowed = false;

            /// <summary>
            /// Clears all the current user's Rich Presence key/values.
            /// </summary>
            public static void ClearRichPresence() => SteamFriends.ClearRichPresence();

            /// <summary>
            /// Retrieves the list of friends followed by the current user asynchronously.
            /// </summary>
            /// <param name="callback">The callback action that receives an array of <c>CSteamID</c> objects representing the followed users.</param>
                ...
                if (callback != null)
                {
                    ...
                    bgWorker.DoWork += (_, e) =>
                    {
                        if (!_loadingFollowed)
                        {
                            ...
                            EnumerateFollowingList(index, (response, isError) =>
                            {
                                if (!isError)
                                {
                                    var ids = response.m_rgSteamID.Where(p => p != CSteamID.Nil);
                                    foreach (var id in ids)
                                        followedIds.Add(id);

                                    total = response.m_nTotalResultCount;
                                    read = response.m_nResultsReturned;
                                }
                                else
                                    hasError = true;
                            ...
                            if (read < total)
                            {
                                ...
                                {
                                    EnumerateFollowingList((uint)read, (responseData, isErrorFlag) =>
                                    {
                                        if (!isErrorFlag)
                                        {
                                            var ids = responseData.m_rgSteamID.Where(p => p != CSteamID.Nil);
                                            foreach (var id in ids)
                                                followedIds.Add(id);

                                            total = responseData.m_nTotalResultCount;
                                            read += responseData.m_nResultsReturned;
                                        }
                                        else
                                            hasError = true;
                            ...
                            _loadingFollowed = false;
                        }
                        else
                        {
                            while (_loadingFollowed)
                                Thread.Sleep(250);
                        }
                    };
                    bgWorker.RunWorkerCompleted += (_, completionEventArgs) =>
                    {
                        callback?.Invoke(completionEventArgs.Result as CSteamID[]);
                        bgWorker.Dispose();
                    };
            ...
            public static void EnumerateFollowingList(uint index, Action<FriendsEnumerateFollowingList_t, bool> callback)
            {
                ...
                _friendsEnumerateFollowingListT ??= CallResult<FriendsEnumerateFollowingList_t>.Create();

                var handle = SteamFriends.EnumerateFollowingList(index);
                _friendsEnumerateFollowingListT.Set(handle, callback.Invoke);
            }
            /// <summary>
            /// Gets the Steam ID of the recently played with user at the given index.
            /// </summary>
            /// <param name="coplayFriendIndex"></param>
            /// <returns></returns>
            ...
            public static void GetFollowerCount(UserData userId, Action<FriendsGetFollowerCount_t, bool> callback)
            {
                ...
                _friendsGetFollowerCountT ??= CallResult<FriendsGetFollowerCount_t>.Create();

                var handle = SteamFriends.GetFollowerCount(userId);
                _friendsGetFollowerCountT.Set(handle, callback.Invoke);
            }
            /// <summary>
            ...
            public static void GetFriendAvatar(CSteamID userId, Action<Texture2D> callback)
            {
                ...
                if (_avatarImageLoadedT == null)
                    _avatarImageLoadedT = Callback<AvatarImageLoaded_t>.Create(HandleAvatarImageLoaded);
                ...
                if (!SteamFriends.RequestUserInformation(userId, false))
                {
                    ...
                    if (handle > 0)
                    {
                        if (_loadedImages.ContainsKey(handle))
            ...
            {
                foreach (var kvp in _loadedImages.Where(kvp => kvp.Value))
                {
                    Object.Destroy(kvp.Value);
                }

                _loadedImages.Clear();
                _userAvatarMapping.Clear();
            }
            /// <summary>
            ...
            {
                var removed = (from kvp in _loadedImages where kvp.Value == image select kvp.Key).ToList();

                var user = CSteamID.Nil;
                foreach (var kvp in _userAvatarMapping.Where(kvp => kvp.Value == image))
                {
                    user = kvp.Key;
                    break;
                }
                ...
                _userAvatarMapping.Remove(user);
            }
            /// <summary>
            ...
            public static void IsFollowing(CSteamID id, Action<FriendsIsFollowing_t, bool> callback)
            {
                ...
                _friendsIsFollowingT ??= CallResult<FriendsIsFollowing_t>.Create();

                var handle = SteamFriends.IsFollowing(id);
                _friendsIsFollowingT.Set(handle, callback.Invoke);
            }
            /// <summary>
            ...
            /// <summary>
            /// Let Steam know that the user is currently using voice chat in the game.
            /// </summary>
            /// <remarks>
            ...
            public static void SetPersonaName(string name, Action<SetPersonaNameResponse_t, bool> callback)
            {
                ...
                if (_mSetPersonaNameResponseT == null)
                    _mSetPersonaNameResponseT = CallResult<SetPersonaNameResponse_t>.Create();

                var handle = SteamFriends.SetPersonaName(name);
                _mSetPersonaNameResponseT.Set(handle, callback.Invoke);
            }
#endif
            /// <summary>
            ...
            /// </summary>
            /// <remarks>
            /// NOTE: The current user must be in the game with the other player for the association to work.
            /// </remarks>
            /// <param name="userId"></param>
            ...
            /// <list type="bullet">
            /// <item>"status"
            /// <para>A UTF-8 string that will show up in the 'view game info' dialogue in the Steam friends list.</para></item>
            /// <item>"connect"
            /// <para>A UTF-8 string that contains the command-line for how a friend can connect to a game. This enables the 'join game' button in the 'view game info' dialog, in the steam friends list right click menu, and on the players Steam community profile. Be sure your app implements <see>
            ///         <cref>App.LaunchCommandLine</cref>
            ///     </see>
            ///     so you can disable the popup warning when launched via a command line.</para></item>
            /// </list>
            /// <para>There are three additional special keys used by the new Steam Chat:</para>
            /// <list type="bullet">
            /// <item>"steam_display"
            /// <para>Names a rich presence localisation token that will be displayed in the viewing user's selected language in the Steam client UI. See Rich Presence Localization for more info, including a link to a page for testing this rich presence data. If steam_display is not set to a valid localization tag, then rich presence will not be displayed in the Steam client.</para></item>
            ...
            /// </list>
            /// <para>
            /// You can clear all the keys for the current user with ClearRichPresence. To get rich presence keys for friends see: GetFriendRichPresence.
            /// </para>
            /// </remarks>
            ...
            /// <item>The user has reached the maximum number of rich presence keys as defined by <see cref="Constants.k_cchMaxRichPresenceKeys"/>.</item>
            /// </list>
            /// </returns>
            public static bool SetRichPresence(string key, string value) => SteamFriends.SetRichPresence(key, value);

            /// <summary>
            /// Retrieves the avatar image associated with the specified user ID if it has been previously loaded.
            /// </summary>
            /// <param name="id">The <see cref="CSteamID"/> of the user whose avatar is being requested.</param>
            /// <returns>A <see cref="Texture2D"/> object representing the user's avatar, or null if the avatar is not loaded.</returns>
            [EditorBrowsable(EditorBrowsableState.Never)]
            public static Texture2D GetLoadedAvatar(CSteamID id)
            {
                return _userAvatarMapping.GetValueOrDefault(id);
            }
            ...
            private static void HandleAvatarImageLoaded(AvatarImageLoaded_t results)
            {
                ...
                {
                    var image = _loadedImages[results.m_iImage];

                    foreach (var links in _pendingLinks.Where(links => links.Owner == results.m_steamID
                                                                       && links.Callback != null))
                    {
                        links.Callback.Invoke(image);
                    }

                    _pendingLinks.RemoveAll(p => p.Owner == results.m_steamID);
                }
                else
                {
                ...
                switch (results.m_nChangeFlags)
                {
                    case EPersonaChange.k_EPersonaChangeAvatar:
                        ...
                        if (avatarData > 0)
                        {
                            ...
                            {
                                var image = _loadedImages[avatarData];
                                foreach (var links in _pendingLinks.Where(links => links.Owner == steamId
                                             && links.Callback != null))
                                {
                                    links.Callback.Invoke(image);
                                }

                                _pendingLinks.RemoveAll(p => p.Owner == steamId);
                            }
                        }
                        break;
                }
            ...
            private static bool LoadAvatar(int imageHandle, CSteamID user)
            {
                if (SteamUtils.GetImageSize(imageHandle, out uint width, out uint height))
                {
                    ...
                    if (_loadedImages.ContainsKey(imageHandle))
                    {
                        pointer = _loadedImages[imageHandle];
                    }
                    ...
                    if (_loadedImages.ContainsKey(imageHandle))
                        ...
                        _loadedImages.Add(imageHandle, pointer);

                    if (_userAvatarMapping.ContainsKey(user))
                        _userAvatarMapping[user] = pointer;
                    else
                        _userAvatarMapping.Add(user, pointer);
            ...
            /// <summary>
            /// Determines whether a specified persona change flag is present in the provided set of persona change values.
            /// </summary>
            /// <param name="value">The combined set of persona change flags to evaluate.</param>
            /// <param name="checkflag">The specific persona change flag to check for.</param>
            ...
            /// <summary>
            /// Checks if the specified persona change value contains all the given flags.
            /// </summary>
            /// <param name="value">The persona change value to examine.</param>
            /// <param name="checkflags">The flags to check against the persona change value.</param>
            /// <returns>True if the persona change value includes all specified flags; otherwise, false.</returns>
